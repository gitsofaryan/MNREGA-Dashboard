...
const GOV_API_URL = process.env.GOV_API_URL || 'https://api.data.gov.in/resource/ee03643a-ee4c-48c2-ac30-9f2ff26ab722';
...
class SyncService {
  async fetchFromGovernmentAPI(state, finYear = null, month = 'April') {
    try {
      ...
      logger.info(`Fetching data from government API: ${state}, ${month}, ${finYear || 'all years'}`);
      ...
      if (response.data && response.data.records) {
        logger.info(`Fetched ${response.data.records.length} records for ${state}`);
      ...
      logger.error(`Error fetching from government API for ${state}:`, error.message);
      throw error;
    }
  }
  ...
  async syncStateData(stateName) {
    try {
      ...
      for (const record of records) {
        try {
          ...
          const mgnregaData = {
            ...
            total_women: parseInt(record.Total_Women_out_of_active_workers || 0),
            avg_days_employment: parseFloat(record.Average_days_of_employment_provided_per_Household || 0),
            hhs_completed_100_days: parseInt(record.Number_of_HHs_completed_100_Days_of_Wage_employment || 0),
            total_households_worked: parseInt(record.Total_No_of_HHs_worked || 0),
            total_individuals_worked: parseInt(record.Total_No_of_Individuals_worked || 0),
            differently_abled_worked: parseInt(record.No_of_Differently_abled_persons_worked || 0),
            ...
            total_work_takenup: parseInt(record.Total_No_of_Works_Takenup_New_Spill || 0),
            ...
            total_administrative_expenditure: parseFloat(record.Total_Adm_Expenditure || 0),
            total_number_persondays_generated: parseInt(record.Total_No_of_Persondays_Generated_so_far || 0),
            persondays_generated_women: parseInt(record.Persondays_Generated_for_Women || 0),
            persondays_generated_sc: parseInt(record.Persondays_Generated_for_SC || 0),
            persondays_generated_st: parseInt(record.Persondays_Generated_for_ST || 0)
          };
      ...
      logger.info(`Sync completed for ${stateName}: ${synced} records synced, ${errors} errors`);
  ...
  async syncAllStatesData() {
    ...
    logger.info(`All states sync completed: ${totalSynced} total records synced, ${totalErrors} total errors`);
